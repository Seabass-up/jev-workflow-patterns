...
- [Initial 32 receipts]({{ '/seo/results/screening.json' | relative_url }})
- [Recovery receipts]({{ '/seo/results/recovery.json' | relative_url }})
...
```sh
python3 seo/evaluate.py
python3 -m unittest discover -s seo -p 'test_*.py' -v
```
...
Successful fixture responses report 23,925 input tokens and 2,067 output tokens. The local bridge estimates their total cost at $0.00100485. That is an estimate, not an invoice, and excludes design-review calls and any billing for failed calls.
