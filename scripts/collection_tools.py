"""Shared offline replay for pattern collections; no network or model calls.

Each collection directory holds catalog.json, fixtures.json, sources.json, and
results/{screening,recovery,summary}.json. Its evaluate.py binds this module to
its own directory. Nothing here calls Jev or regenerates missing results.
"""
import hashlib
import json
import math
...
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "skills/jev-question-kernel-catalog/scripts"))
...
def report(root):
    return replay(read(root, "catalog.json"), read(root, "fixtures.json"), read(root, "sources.json"),
                  read(root, "results/screening.json"), read(root, "results/recovery.json"))


def run(root):
    result = report(root)
    print(json.dumps(result, indent=2))
    if (Path(root) / "results/summary.json").exists() and result != read(root, "results/summary.json"):
